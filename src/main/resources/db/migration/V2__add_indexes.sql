...
-- Ritual history
CREATE INDEX idx_ritual_history_user_updated_at
    ON ritual_history (user_id, updated_at DESC);

CREATE INDEX idx_ritual_history_user_status_updated_at
    ON ritual_history (user_id, status, updated_at DESC);

-- Ritual recommendations
CREATE INDEX idx_ritual_recommendations_user_created_at
    ON ritual_recommendations (user_id, created_at DESC);

-- User context
CREATE INDEX idx_user_contexts_user_conversation
    ON user_contexts (user_id, conversation_id);

-- Rituals
CREATE INDEX idx_rituals_published_updated_at
    ON rituals (updated_at DESC)
    WHERE status = 'PUBLISHED';

CREATE INDEX idx_rituals_published_title
    ON rituals (title)
    WHERE status = 'PUBLISHED';

CREATE EXTENSION pg_trgm;
 
CREATE INDEX idx_rituals_search_trgm
    ON rituals 
